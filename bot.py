...
if __name__ == '__main__':
    ...
    bot.voice_channel_mgr = voiceChannelManager.VoiceChannelManager(bot)
...
Customize notification/bot management of the server. Configurations you can make:
    ...
    ~set_config onDemandVoiceChannelPerGame (Add a voice channel called 'Create For Current Game' which creates a voice channel for the game you're currently playing)
''')
async def set_config(ctx, key: str, value: bool):
    ...
    if key in bot.guildConfigs.supportedConfigKeys:
        try:
            ...
            await bot.voice_channel_mgr.on_guild_configs_updated(ctx.guild)
            await ctx.send(f'Successfully set config:\n\n{key_value_message_format.format(key, set_val)}')
        except:
            await ctx.send('Sorry, there was an internal problem. Please join the support server for help. :(')
    else:
...
Get custom configurations of the server. Configurations you can get:
    ...
    ~get_config onDemandVoiceChannelPerGame (Add a voice channel called 'Create For Current Game' which creates a voice channel for the game you're currently playing)
''')
